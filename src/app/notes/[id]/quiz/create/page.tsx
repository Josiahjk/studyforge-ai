...
import { NoteQuizClient, type NoteQuizQuestion } from "@/components/notes/note-quiz-client";
...
export default async function GeneratedNoteQuizCreatePage({ params }: PageProps) {
  ...
  return (
    <AppShell user={shellUser(user)}>
      <PageHeader title="Create Quiz" body={documentTitle} />
      <NoteQuizClient noteId={generation.id} title={documentTitle} sourceNotes={sourceNotes} initialQuestions={initialQuestions} mode="create" />
    </AppShell>
  );
}
